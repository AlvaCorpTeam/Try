...
def caesar(text, shift, direction):
    ...
    if shift >= alpha_len:                   # check what if the shift number is bigger than the length of the alpha list
        ...
        # Calculate number that approach the shift number so as the shift number reduced by, the number inside range index of alpa_list obtained
        shift = shift - (reset * alpha_len)

    # decide to shift forward (encode) or backward(decode)
    if direction == 'decode':
        shift *= -1

    for letter in text:
        # keep letter index based on alpha list
        idx_alpa = alphabet.index(letter)
